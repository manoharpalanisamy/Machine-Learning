...
% Foward propagation
% a1 = X;
X = [ones(m,1) X]; % 5000*401  add bias term
z2 = Theta1 * X'; % (25*401)*(401*5000)
a2 = sigmoid(z2); % (25*5000)

a2 = [ones(m,1) a2']; % add bias term (5000X26)
z3 = Theta2 * a2';    % (10X26) * (5000X26)' -> (10X5000)
h_theta = sigmoid(z3); % h_theta equals a3 (10X5000)
...
t1 = Theta1(:,2:size(Theta1,2)); % 2:401 (25X400)
t2 = Theta2(:,2:size(Theta2,2)); % 2:26  (10X25)

% Regularization
Reg = lambda  * (sum( sum ( t1.^ 2 )) + sum( sum ( t2.^ 2 ))) / (2*m);  % real no 
